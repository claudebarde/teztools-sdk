...
let $882b6d93070905b3$var$tokenType;
(function(tokenType) {
    tokenType["FA12"] = "fa1.2";
    tokenType["FA2"] = "fa2";
    tokenType["XTZ"] = "XTZ";
})($882b6d93070905b3$var$tokenType || ($882b6d93070905b3$var$tokenType = {}));
const $882b6d93070905b3$var$validateAddress = (address)=>{
...
class $882b6d93070905b3$export$2e2bcd8739ae039 {
   ...
   */ async setup(p) {
        ...
        const fiatExchangeRate = p && p.fiatExchangeRate ? p.fiatExchangeRate : undefined;
        if (prices) // fetches tokens prices
        try {
            const response = await fetch(this.pricesApiUrl);
            if (response) {
                const data = await response.json();
                if (typeof data !== "object") throw `Expected object from token prices API, got ${typeof data}`;
                else if (typeof data === "object" && !data.hasOwnProperty("contracts")) throw `Object received from token prices API doesn't have a "contracts" property`;
                else if (typeof data === "object" && data.hasOwnProperty("contracts") && !Array.isArray(data.contracts)) throw `Expected contracts property from token prices API to be an array, got ${typeof data.contracts} instead`;
                else {
                    // right format
                            ...
                            switch(contractKey){
                                case "symbol":
                                    ...
                                    if (this.is_string(symbol)) {
                                        ...
                                        token.symbol = symbol;
                                    } else token.symbol = null;
                                    break;
                                case "tokenAddress":
                                    const tokenAddress = contract[contractKey];
                                    token.tokenAddress = (()=>{
                                        if (this.is_string(tokenAddress) && $882b6d93070905b3$var$validateAddress(tokenAddress)) return {
                                ...
                                case "address":
                                    ...
                                    token.address = (()=>{
                                        if (this.is_string(address) && $882b6d93070905b3$var$validateAddress(address)) return {
                                            address: address,
                                            isValid: true
                                        };
                                        else return null;
                                ...
                                case "type":
                                    ...
                                    switch(type){
                                        case $882b6d93070905b3$var$tokenType.FA12:
                                            token.type = $882b6d93070905b3$var$tokenType.FA12;
                                            break;
                                        case $882b6d93070905b3$var$tokenType.FA2:
                                            token.type = $882b6d93070905b3$var$tokenType.FA2;
                                            break;
                                        case $882b6d93070905b3$var$tokenType.XTZ:
                                            token.type = $882b6d93070905b3$var$tokenType.XTZ;
                                            break;
                                        default:
                                            token.type = null;
                                            break;
                                ...
                                case "bakerValidator":
                                    ...
                                    token.bakerValidator = $882b6d93070905b3$var$validateAddress(bakerValidator) ? bakerValidator : null;
                                    break;
                                case "currentCandidate":
                                    const currentCandidate = contract[contractKey];
                                    token.currentCandidate = $882b6d93070905b3$var$validateAddress(currentCandidate) ? currentCandidate : null;
                                    break;
                                case "currentDelegated":
                                    const currentDelegated = contract[contractKey];
                                    token.currentDelegated = $882b6d93070905b3$var$validateAddress(currentDelegated) ? currentDelegated : null;
                                    break;
                                case "lastUpdateTime":
                                ...
                                case "pairs":
                                    ...
                                    pairs.forEach((pair)=>{
                                        let tokenPair = {
                                            address: $882b6d93070905b3$var$validateAddress(pair.address) ? pair.address : null,
                                ...
                                case "websiteLink":
                                    ...
                                    token.websiteLink = (()=>{
                                        if (this.is_string(websiteLink)) return {
                                            url: websiteLink,
                                ...
                                case "telegramLink":
                                    ...
                                    token.telegramLink = (()=>{
                                        if (this.is_string(telegramLink)) return {
                                            url: telegramLink,
                                ...
                                case "twitterLink":
                                    ...
                                    token.twitterLink = (()=>{
                                        if (this.is_string(twitterLink)) return {
                                            url: twitterLink,
                                ...
                                case "discordLink":
                                    ...
                                    token.discordLink = (()=>{
                                        if (this.is_string(discordLink)) return {
                                            url: discordLink,
                                ...
                                case "thumbnailUri":
                                    ...
                                    token.thumbnailUri = (()=>{
                                        if (this.is_string(thumbnailUri)) return {
                                            url: thumbnailUri,
                    ...
                    this.tokenTags = Array.from(new Set(this.tokensPrices.map((tk)=>tk.tags
                    ).filter((tk)=>tk !== null
                    )));
                }
            } else throw "No response from the tokens prices API";
        } catch (error) {
            console.error();
        }
        if (xtzPrice) // fetches XTZ price
        try {
            ...
            if (response) {
                ...
                const matchingKeys = receivedKeys.map((key)=>expectedKeys.includes(key)
                ).reduce((a, b)=>a && b
                );
                if (!matchingKeys || expectedKeys.length === receivedKeys.length) xtzPrice.fullData = false;
                // copies received data into xtzPrice object
                Object.keys(xtzPrice).forEach((key)=>{
                    if (data.hasOwnProperty(key)) {
                        // checks data type
                        if (key === "updated" && this.is_timestamp(data[key])) xtzPrice.updated = data.updated;
                        else if (this.is_number(data[key])) // HACK: find a proper way to type xtzPrice[key]
                ...
                } else this.xtzExchangeRate = xtzPrice.price;
            } else throw "No response from the XTZ price API";
        } catch (error1) {
   ...
   */ get(tokenSymbol) {
        if (!this.tokensList.includes(tokenSymbol)) return null;
        else {
   ...
   */ getByAddress(address) {
        const result = this.tokensPrices.find((tk)=>tk.tokenAddress ? tk.tokenAddress.address === address : undefined
        );
        if (result) return result;
        else return null;
    }
   ...
   */ getByTag(tag, precision) {
        if (precision) return this.tokensPrices.filter((tk)=>tk.tags && tk.tags === tag.toLowerCase()
        );
        else return this.tokensPrices.filter((tk)=>tk.tags && tk.tags.toLowerCase().includes(tag.toLowerCase())
        );
    }
    /**
   ...
   */ getTokensWithPriceGreaterThan(price) {
        return this.tokensPrices.filter((tk)=>tk.currentPrice ? tk.currentPrice > price : false
        ).sort((a, b)=>{
   ...
   */ orderByUsdValue(dir, tokens) {
        const sortedTokens = this.tokensPrices.sort((a, b)=>{
            if (a.usdValue && b.usdValue) return dir === "desc" ? b.usdValue - a.usdValue : a.usdValue - b.usdValue;
            else return 0;
        });
        if (tokens) return sortedTokens.filter((tk)=>tk.symbol ? tokens.includes(tk.symbol) : false
        );
        else return sortedTokens;
    }
   ...
   */ getCurrentPrice(tokens) {
        ...
        if (tokens && tokens.length > 0) return tokensCurrentPrices.filter((tk)=>tk.symbol ? tokens.includes(tk.symbol) : false
        );
        else return null;
    }
}
